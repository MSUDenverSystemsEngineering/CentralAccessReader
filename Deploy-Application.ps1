...
Try {
	...
	[string]$appVendor = 'Central Washington University'
	[string]$appName = 'Central Access Reader'
	[string]$appVersion = '2.20'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		$exitCode = Execute-Process -Path "$dirFiles\CAR_Setup_64.exe" -Parameters "/verysilent /suppressmsgboxes /norestart" -WindowStyle "Hidden" -PassThru
		...
		$exitCode = Execute-Process -Path "$dirSupportFiles\toolbar\setup.exe" -Parameters "-componenturl" -PassThru
        If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }

        ##  Install CAR Check
        $exitCode = Execute-Process -Path "$dirSupportFiles\check\setup.exe" -Parameters "-componenturl" -PassThru
        If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$exitCode = Execute-Process -Path "C:\Program Files\Central Access Reader\unins000.exe" -Parameters "/SILENT" -PassThru
        If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
